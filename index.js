...
const API_KEY =
  "live_bEEp5ex9VnAtfmE13pJLIKoqvun03uda4aH4QzwQQphvoWlci2LLLgYTIWMCYrXx";
...
export async function favourite(imgId) {
  ...
  // The modified initialLoad function with Axios
  async function initialLoadWithAxios() {
    ...
    try {
      ...
      breedSelect.addEventListener("change", async (event) => {
        ...
        try {
          const { data: breedImages } = await axios.get(
            `images/search?breed_id=${selectedBreed}&limit=5`
          );
          ...
          breedImages.forEach((image) => {
            ...
            imgElement.className = "carousel-image"; // Add any class for styling
          ...
          const infoHTML = `<h2>${breedInfo.name}</h2><p>${breedInfo.temperament}</p>`;
      ...
      // Trigger change event to load initial carousel
